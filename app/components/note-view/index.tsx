...
export function NoteView({ note }: NoteProps) {
  ...
  return (
    <div className="shadow-black-500/50 relative box-border h-64 w-64 border-4 bg-yellow-100 p-4 shadow-lg">
      ...
      <div className="absolute inset-x-0 bottom-4 left-1 grid place-items-center">
        <p className="text-xs font-extralight">{formatDate(createdAt.toISOString())}</p>
      </div>
    </div>
  );
}
